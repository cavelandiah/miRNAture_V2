...
sub generate_file_mirnature {
    ...
    my $outfile = $variable->[4]->{"User_results"}->{"Output_miRNAture_folder"}."/mirnature_run_".$variable->[3]->{Species_data}->{Tag}."_".$variable->[3]->{Homology_options}->{Mode};
...
sub build_cmsearch_cli_parameters {
	my $homology_options = shift;
	my @parameters;
	foreach my $option (qw(cmsearch_evalue cmsearch_profile cmsearch_cpu cmsearch_jobs cmsearch_extra_args)){
		next if !exists $homology_options->{$option};
		next if !defined $homology_options->{$option} || $homology_options->{$option} eq "";
		push @parameters, "--$option \"$homology_options->{$option}\"";
	}
...
sub clean_cache {
    ...
    my $index = $variables->[3]->{Species_data}->{Old_Genome}.".index";
...
sub move_results {
	...
	_move_to_results($folder."/miRNAture_configuration_".$variables->[3]->{Species_data}{"Tag"}.".yaml", $output);
}
